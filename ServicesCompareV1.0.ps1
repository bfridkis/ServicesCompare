        ...
        if (![string]::IsNullOrEmpty($fileName)) {
            ...
            if ($pathIsValid) {
                ...
                if ($fileAlreadyExists) {
                    ...
                    } while ($overWriteConfirmed.ToUpper() -ne "Y" -and $overWriteConfirmed.ToUpper() -ne "N")
                }
            }
...
#                MismatchFileName: $MismatchFileName AllServicesFileName: $AllServicesFileName`n`n"
([string]$args).split('-') | %{ 
                                ...
                                elseif ($_.Split(' ')[0].ToUpper() -eq "MismatchFileName") { $MismatchFileName = $_.Split(' ')[1] }
                                elseif ($_.Split(' ')[0].ToUpper() -eq "AllServicesFileName") { $AllServicesFileName = $_.Split(' ')[1] }
                              }
#write-output "Node1: $Node1, Node2: $Node2, OutputMode: $OutputMode, ShowAllServices: $ShowAllServices, Property: $Property 
#                MismatchFileName: $MismatchFileName AllServicesFileName: $AllServicesFileName`n`n"
...
if (!$OutputMode) { do { $OutputMode = read-host -prompt "`nSave To File (1), Console Output (2), or Both (3)" ;  if (([string]$OutputMode).ToUpper() -eq "Q") { exit } } 
                    while ($OutputMode -ne 1 -and $OutputMode -ne 2 -and $OutputMode -ne 3) }

if ($ShowAllServices -and $ShowAllServices.ToUpper() -ne "FALSE" -and ($OutputMode -eq 1 -or $OutputMode -eq 3)) { 
...
if ($misMatches -and $OutputMode -in 2..3) { 
    ...
    $mismatches | select-object @{ n = 'Service' ; e = {$_.Service}},
                                ...
                                @{ n = "$($Node2_Property)" ; e = {$(if ($_.$($Node2_Property)) { $_.$($Node2_Property) } else { "[Service Not Found]"})}} -OutVariable MismatchExport
}
else {
     $mismatches | select-object @{ n = 'Service' ; e = {$_.Service}},
                                 @{ n = "$($Node1_Property)" ; e = {$(if ($_.$($Node1_Property)) { $_.$($Node1_Property) } else { "[Service Not Found]"})}},
...
if ($ShowAllServices -and $ShowAllServices.ToUpper() -ne "FALSE" -and $OutputMode -in 2..3) {
...
elseif ($ShowAllServices -and $ShowAllServices.ToUpper() -ne "FALSE") {
    $results | select-object @{ n = "Name" ; e = {$_.Name}},
                             @{ n = "Result_Set" ; e = { $( if($_.Group[0].MachineName -and $_.Group[1].MachineName -and $_.Group[0].MachineName -eq $Node1) 
                                                            { "@{MachineName=$($_.Group[0].MachineName); $($Property)=$($_.Group[0].$($Property))}, @{MachineName=$($_.Group[1].MachineName); $($Property)=$($_.Group[1].$($Property))}" }  
                                                            elseif($_.Group[0].MachineName -and $_.Group[1].MachineName)
...
if ($OutputMode -eq 1 -or $OutputMode -eq 3) {
    ...
    if ($ShowAllServices -and $ShowAllServices.ToUpper() -ne "FALSE") { $AllServicesExport | export-CSV -Path $fileNameForAllServices -NoTypeInformation }
}
